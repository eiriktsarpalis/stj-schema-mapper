...
public static class JsonSchemaMapper
{
        ...
        JsonNumberHandling? customNumberHandling = null)
    {
        ...
        if (TryGetNullableElement(typeInfo.Type, out Type? nullableElementType))
        {
            ...
            if (elementSchema.TryGetPropertyValue(EnumPropertyName, out JsonNode? nullaleElementEnumValues))
            {
                // Special case nullable string enum types which require inserting the null value to the "enum" property
